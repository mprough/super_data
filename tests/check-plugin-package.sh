...
version_dir='files/zc_plugins/SuperData/v3.0.12'
...
required=(
    ...
    'files/legacy/sql/upgrade_to_3.0.12.sql'
...
grep -Fq "'pluginVersion' => 'v3.0.12'" "$manifest"
...
grep -Fq 'protected function getOrCreateConfigGroupId(' "$installer"
...
grep -Fq "'PLUGIN_SUPERDATA_VERSION', '3.0.12'" "$installer"
grep -Fq "array(\'RateTables\', \'Free\', \'FlatRate\')" "$installer"
grep -Fq "define('BOX_CONFIGURATION_SUPER_DATA', 'SuperData v3.0.12')" \
    "$version_dir/admin/includes/extra_datafiles/super_data_menu_name.php"
grep -Fq "define('BOX_CONFIGURATION_SUPER_DATA', 'SuperData v3.0.12')" \
    'files/legacy/YOUR_ADMIN/includes/extra_datafiles/super_data.php'
...
configuration_js="$version_dir/admin/includes/javascript/configuration.php"
grep -Fq "PLUGIN_SUPERDATA_VERSION" "$configuration_js"
grep -Fq "form.addEventListener('submit'" "$configuration_js"
grep -Fq 'originalField.disabled = true' "$configuration_js"
