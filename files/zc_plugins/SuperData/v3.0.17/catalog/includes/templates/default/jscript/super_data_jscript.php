...
/**
 * Format a decimal for JSON-LD without exposing binary floating point noise.
 * Schema.org QuantitativeValue accepts a numeric string.
 */
function sdata_decimal_value($value, int $precision = 4): string
{
...
/**
 * Resolve the effective product country of origin.
 *
 * Google Product Loader supplies the preferred resolver. The local fallback
 * supports its products-table field and store default when the helper is not
 * loaded, while keeping SuperData usable without that plugin.
 */
function sdata_get_product_country_of_origin(int $productId): array
{
    global $db, $sniffer;

    if (function_exists('gpsf_get_product_country_of_origin')) {
        $country = gpsf_get_product_country_of_origin($productId);
    ...
    $countryId = 0;
    $countryField = defined('GPSF_PRODUCT_FIELD_COUNTRY_OF_ORIGIN')
        ? (string)GPSF_PRODUCT_FIELD_COUNTRY_OF_ORIGIN
        : 'products_country_of_origin';

    if (preg_match('/^[a-zA-Z0-9_]+$/', $countryField)
        && isset($sniffer)
        && $sniffer->field_exists(TABLE_PRODUCTS, $countryField)) {
        $productCountry = $db->Execute(
            'SELECT ' . $countryField . '
               FROM ' . TABLE_PRODUCTS . '
              WHERE products_id = ' . $productId . '
              LIMIT 1'
        );
        if (!$productCountry->EOF) {
            $countryId = (int)($productCountry->fields[$countryField] ?? 0);
        }
    }
    ...
    if ($countryId < 1) {
        ...
        } elseif (defined('PLUGIN_SUPERDATA_COUNTRY_OF_ORIGIN_DEFAULT')) {
            $countryId = (int)PLUGIN_SUPERDATA_COUNTRY_OF_ORIGIN_DEFAULT;
        }
    }
    if ($countryId < 1) {
        return [];
    ...
    if ($country->EOF || empty($country->fields['countries_name'])) {
        return [];
    }

    return [
        'id' => $countryId,
        'name' => $country->fields['countries_name'],
        'iso2' => $country->fields['countries_iso_code_2'] ?? '',
        'iso3' => $country->fields['countries_iso_code_3'] ?? '',
    ];
}
...
switch ($page_type) {
    case 'product':
        ...
        $product_priced_by_attributes = zen_get_products_price_is_priced_by_attributes($product_id);
        ...
        // Country of origin, using the same effective value as Google Product Loader when available.
        $product_country_of_origin = sdata_get_product_country_of_origin((int)$product_id);
...
// Get attribute info
            ...
            foreach ($results as $attribute) {
                if (zen_get_attributes_valid($product_id, $attribute['options_id'], $attribute['options_values_id'])) { // skips "display only"
                    ...
                    // A raw options_values_price is only one component of a priced-by-attributes
                    // product. Let Zen Cart calculate the complete single-option variant price;
                    // the default (including multi-option) path below uses Zen Cart's calculated
                    // "Starting at" product price instead of presenting components as offers.
                    if ($product_priced_by_attributes) {
                        $attribute_displayed_price = (float)zen_get_attributes_price_final(
                            (int)$attribute['products_attributes_id'],
                            1,
                            null,
                            false,
                            true,
                        ...
                        $product_attributes[$attribute['products_attributes_id']]['price'] = round($attribute_displayed_price, 2);
                    } else {
                        $product_attributes[$attribute['products_attributes_id']]['price'] = $product_base_displayed_price;
                    }
            ...
            if ($debug_sd) {
                echo __LINE__ . ' count($product_attributes)=' . count($product_attributes);
...
// Product schema for product pages
    if ($page_type === 'product') {
        ...
        $productSchema = [
            ...
            'weight' => [
                '@type' => 'QuantitativeValue',
                'value' => sdata_decimal_value($weight),
        ...
        if (!empty($product_country_of_origin['name'])) {
            $productSchema['countryOfOrigin'] = [
                '@type' => 'Country',
                'name' => $product_country_of_origin['name'],
            ];
        }
        ...
        if ($product_attributes) {
            ...
            switch ($attribute_stock_handler) {
                ...
                case 'posm':
                    ...
                    foreach ($product_attributes as $index => $product_attribute) {
                        ...
                        $offer = [
                            ...
                            'weight' => [
                                '@type' => 'QuantitativeValue',
                                'value' => sdata_decimal_value(
                                                $weight + $product_attribute['weight'] > 0
                ...
                default:
                    ...
                    // zen_get_products_actual_price() already supplies Zen Cart's complete
                    // "Starting at" price, including the lowest base-included value from each
                    // required option group. Individual attribute rows are not purchasable offers.
                    $offer['@type'] = 'Offer';
                    $offer['price'] = number_format((float)$product_base_displayed_price, $decimal_places, '.', '');
